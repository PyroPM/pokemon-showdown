...
export const Chat = new class {
	...
	getDataPokemonHTML(species: Species, gen = 8, tier = '') {
		...
		if (species.types) {
			for (const type of species.types) {
				buf += `<img src="https://${Config.routes.uranium_client}/sprites/types/${type}.png" alt="${type}" height="14" width="32">`;
			}
		}
		buf += '</span> ';
		if (gen >= 3) {
	...
	getDataMoveHTML(move: Move, isChampions = false) {
		...
		buf += `<span class="col typecol"><img src="//${Config.routes.uranium_client}/sprites/types/${encodedMoveType}.png" alt="${move.type}" width="32" height="14">`;
		buf += `<img src="//${Config.routes.uranium_client}/sprites/categories/${move.category}.png" alt="${move.category}" width="32" height="14"></span> `;
		if (move.basePower) {

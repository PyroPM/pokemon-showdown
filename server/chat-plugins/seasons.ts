...
function getUserHTML(user: User, format: string) {
	...
	if (badgeType) {
		...
		return `<img src="https://${Config.routes.uranium_client}/sprites/misc/${formatType}_${badgeType}.png" />` + buf;
	}
	return buf;
}
...
export const pages: Chat.PageTable = {
	...
	seasonladder(query, user) {
		...
		for (const badgeType in data.badgeholders[season][format]) {
			...
			let formatType = format.split(/gen\d+/)[1];
			if (!['ou', 'randombattle'].includes(formatType)) formatType = 'rotating';
			buf += `<tr><h2><img src="https://${Config.routes.uranium_client}/sprites/misc/${formatType}_${badgeType}.png" /> ${uppercase(badgeType)}</h2></tr>`;

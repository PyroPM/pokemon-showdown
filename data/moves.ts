...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	camouflage: {
		...
		onHit(target) {
			...
			if (this.field.isTerrain('electricterrain')) {
				...
				newType = 'Psychic';
			} else if (this.field.isTerrain('stickyterrain')) {
				newType = 'Poison';
			}
	...
	dig: {
		...
		condition: {
			...
			onImmunity(type, pokemon) {
				if (type === 'sandstorm' || type === 'hail' || type === 'thunderstorm' || type === 'fallout' || type === 'chernobyl') return false;
			},
			onInvulnerability(target, source, move) {
	...
	solarbeam: {
		...
		onBasePower(basePower, pokemon, target) {
			const weakWeathers = ['raindance', 'primordialsea', 'sandstorm', 'hail', 'snowscape', 'thunderstorm', 'fallout', 'chernobyl'];
	...
	expunge: {
		...
		onModifyMove(move) {
			if (this.field.isWeather('fallout') || this.field.isWeather('chernobyl')) move.accuracy = true;
		},
		target: "normal",
		type: "Nuclear",
		contestType: "Cool",
